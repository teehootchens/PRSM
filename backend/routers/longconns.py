...
def get_longconns(
    ...
    conditions = [f"long_conn_score >= %(min_score)s", "long_conn_score > 0"]
    ...
    query = f"""
        SELECT
            ...
            fqdn, long_conn_score, beacon_threat_score,
            total_duration, count, total_bytes, last_seen,
            port_proto_service, threat_intel, ts_intervals,
            ts_interval_counts, ds_sizes, ds_size_counts, analyzed_at,
            beacon_score, modifier_name, modifier_value,
            c2_over_dns_score, strobe_score, prevalence,
            prevalence_score, prevalence_total, network_size,
            missing_host_count, first_seen_score
