...
if $OFFLINE; then
    ...
    for spec in "fastapi:fastapi" "uvicorn:uvicorn" "clickhouse_connect:clickhouse-connect" \
                "passlib:passlib" "dotenv:python-dotenv" "multipart:python-multipart" "openpyxl:openpyxl"; do
        imp="${spec%%:*}"
        pkg="${spec##*:}"
    ...
    # Check bcrypt specifically (part of passlib extras)
    if ! "$PYTHON_BIN" -c "from passlib.hash import bcrypt" 2>/dev/null; then
        offline_missing+=("passlib[bcrypt]")
        fail "Python package missing (offline mode): passlib[bcrypt] — bcrypt support unavailable"
    fi
...
if [ -z "$GUI_PASSWORD" ]; then
    while true; do
        read -r -s -p "  GUI password: " GUI_PASSWORD
        echo ""
...
if $OFFLINE; then
    ...
    pip3 install \
        ...
        openpyxl "passlib[bcrypt]" python-dotenv \
...
if [ -f "$INSTALL_DIR/.env" ] && ! $FORCE; then
    ...
    GUI_PASSWORD_HASH=$("$PYTHON_BIN" -c \
        "from passlib.hash import bcrypt; print(bcrypt.hash('${GUI_PASSWORD}'))")
